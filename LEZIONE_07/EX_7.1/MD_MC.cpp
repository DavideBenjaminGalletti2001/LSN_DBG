...
void Control(int argc, char** argv){
    cout << "I FILE .dat SONO STATI RIMOSSI? (./clean<phase>.sh)? [y/N]" << endl;
    string resp;
    cin >> resp;
    if (resp == "N" or resp == "n" or resp == "NO" or resp == "no"){throw runtime_error("Warning, you have to remove .dat files!!!");};
    ...
    { 
        ...
        {
            if (tent > 0) cout << "FASE NON NOTA: " << endl;
            cout << "INSERIRE UNA FASE VALIDA: (O DIRETTAMENTE SCRIVERE \"./NVE_NVT.exe <phase>\"): SOLID, LIQUID or GAS? ";
            cin >> PHASE;
            tent ++;
        }
    }
    ...
    { 
        ...
        { 
            throw runtime_error("SI USI: ./NVE_NVT.exe <phase>, CON FASE = {solid, liquid, gas}.");
        }
    } 
}
...
void Input(void)
{
    ...
    ReadInput.open("INPUT_FILE/input." + PHASE);
   
    if(ReadInput.fail()) throw runtime_error("Unable to open file input.<phase> :" + PHASE +" "+ phase);
    ...
    {
        if(iNVET){
            ReadConf.open("OUTPUT_FILE/MC/"+ PHASE + "_FILE/EQUILIBRAZIONE/config.out");
            ReadVelocity.open("OUTPUT_FILE/MC/" + PHASE + "_FILE/EQUILIBRAZIONE/velocity.out");
            for (int i=0; i<npart; ++i) ReadVelocity >> vx[i] >> vy[i] >> vz[i];
        }else if (!iNVET){
            ReadConf.open("OUTPUT_FILE/MD/"+ PHASE + "_FILE/EQUILIBRAZIONE/config.out");
            ReadVelocity.open("OUTPUT_FILE/MD/" + PHASE + "_FILE/EQUILIBRAZIONE/velocity.out");
            for (int i=0; i<npart; ++i) ReadVelocity >> vx[i] >> vy[i] >> vz[i];
        }
    }
    else 
    {
        ReadConf.open("INPUT_FILE/config.in");
    ...
    cout << "| Initial pressure         = " << walker[iw]/(3.0*vol) + rho*temp << endl;
    cout << "$----------------------------------------$" << endl;
...
void Averages(int iblk) //Print results for current block
{
    ...
    if (iNVET == 1){
        Epot.open("OUTPUT_FILE/MC/"+ PHASE + "_FILE/output_epot.dat",ios::app);
        Ekin.open("OUTPUT_FILE/MC/"+ PHASE + "_FILE/output_ekin.dat",ios::app);
        Temp.open("OUTPUT_FILE/MC/"+ PHASE + "_FILE/output_temp.dat",ios::app);
        Etot.open("OUTPUT_FILE/MC/"+ PHASE + "_FILE/output_etot.dat",ios::app);
        Pres.open("OUTPUT_FILE/MC/"+ PHASE + "_FILE/output_pres.dat",ios::app);
        Gdr.open("OUTPUT_FILE/MC/"+ PHASE + "_FILE/output_gdr.dat",ios::app);
    }else{
        Epot.open("OUTPUT_FILE/MD/"+ PHASE + "_FILE/output_epot.dat",ios::app);
        Ekin.open("OUTPUT_FILE/MD/"+ PHASE + "_FILE/output_ekin.dat",ios::app);
        Temp.open("OUTPUT_FILE/MD/"+ PHASE + "_FILE/output_temp.dat",ios::app);
        Etot.open("OUTPUT_FILE/MD/"+ PHASE + "_FILE/output_etot.dat",ios::app);
        Pres.open("OUTPUT_FILE/MD/"+ PHASE + "_FILE/output_pres.dat",ios::app);
        Gdr.open("OUTPUT_FILE/MD/"+ PHASE + "_FILE/output_gdr.dat",ios::app);
    }
    /*
    if (Epot.fail() || Ekin.fail() || Temp.fail() || Etot.fail() || Pres.fail() || Gdr.fail())
    {
      cout << "unable to open output file" << endl;
      throw runtime_error("UNABLE OPEN FILES!!!");
    }
    */
    ...
    stima_pres = blk_av[iw]/blk_norm; //Pressure: pressuere is a intensive observable
    ...
    //RISULTATI: POTENZIALE, ENERGIA CINETICA, ENERGIA TOTALE TEMPERATURA, PRESSIONE

    // NUMERO BLOCCHI(MISURE STATISTICAMENTE SCORRELATE) -> VALOR MEDIO SINGOLO BLOCCO -> MEDIA PROGRESSIVA -> ERRORE PROGRESSIVO
...
void ConfFinal(void)
{
  ...
  if (iNVET){
    WriteConf.open("OUTPUT_FILE/MC/"+PHASE+"_FILE/config.out");
    WriteVelocity.open("OUTPUT_FILE/MC/"+PHASE+"_FILE/velocity.out");
  }else{
    WriteConf.open("OUTPUT_FILE/MD/"+PHASE+"_FILE/config.out");
    WriteVelocity.open("OUTPUT_FILE/MD/"+PHASE+"_FILE/velocity.out");
  }
  for (int i=0; i<npart; ++i)
  {
...
void ConfXYZ(int nconf){ //Write configuration in .xyz format
  ...
  WriteXYZ << npart << endl;
  WriteXYZ << "MyData:" << endl;//This is only a comment
  for (int i=0; i<npart; ++i){
